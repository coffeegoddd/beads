...
func TestExternalDoltConfigValidate(t *testing.T) {
	t.Run("tcp endpoint with host+port", func(t *testing.T) {
		require.NoError(t, ExternalDoltConfig{Host: "db.internal", Port: 3306}.Validate())
	})

	t.Run("unix socket endpoint", func(t *testing.T) {
		require.NoError(t, ExternalDoltConfig{Socket: "/var/run/dolt.sock"}.Validate())
	})

	t.Run("tcp endpoint with tls required and no cert/key", func(t *testing.T) {
		require.NoError(t, ExternalDoltConfig{
	...
	t.Run("negative keep alive period rejected", func(t *testing.T) {
		err := ExternalDoltConfig{Host: "db", Port: 3306, KeepAlivePeriod: -1 * time.Second}.Validate()
		require.Error(t, err)
...
func TestNewExternalDoltServer_RejectsInvalidConfig(t *testing.T) {
	_, err := NewExternalDoltServer(ExternalDoltConfig{})
	require.Error(t, err)
}

func TestExternalDoltServer_ID(t *testing.T) {
	t.Run("same tcp endpoint produces same id", func(t *testing.T) {
		a, err := NewExternalDoltServer(ExternalDoltConfig{Host: "db", Port: 3306})
		require.NoError(t, err)
		b, err := NewExternalDoltServer(ExternalDoltConfig{Host: "db", Port: 3306})
		require.NoError(t, err)
	...
	t.Run("different ports produce different ids", func(t *testing.T) {
		a, err := NewExternalDoltServer(ExternalDoltConfig{Host: "db", Port: 3306})
		require.NoError(t, err)
		b, err := NewExternalDoltServer(ExternalDoltConfig{Host: "db", Port: 3307})
		require.NoError(t, err)
	...
	t.Run("different hosts produce different ids", func(t *testing.T) {
		a, err := NewExternalDoltServer(ExternalDoltConfig{Host: "db-a", Port: 3306})
		require.NoError(t, err)
		b, err := NewExternalDoltServer(ExternalDoltConfig{Host: "db-b", Port: 3306})
		require.NoError(t, err)
		assert.NotEqual(t, a.ID(context.Background()), b.ID(context.Background()))
	})
	...
	t.Run("socket and tcp produce different ids even when notation overlaps", func(t *testing.T) {
		a, err := NewExternalDoltServer(ExternalDoltConfig{Host: "db", Port: 3306})
		require.NoError(t, err)
		b, err := NewExternalDoltServer(ExternalDoltConfig{Socket: "/var/run/dolt.sock"})
		require.NoError(t, err)
		assert.NotEqual(t, a.ID(context.Background()), b.ID(context.Background()))
	})
}
...
func TestExternalDoltServer_DSN(t *testing.T) {
	t.Run("tcp without tls", func(t *testing.T) {
		s, err := NewExternalDoltServer(ExternalDoltConfig{Host: "db", Port: 3306})
		require.NoError(t, err)
	...
	t.Run("tcp with tls required", func(t *testing.T) {
		s, err := NewExternalDoltServer(ExternalDoltConfig{Host: "db", Port: 3306, TLSRequired: true})
		require.NoError(t, err)
	...
	t.Run("unix socket", func(t *testing.T) {
		s, err := NewExternalDoltServer(ExternalDoltConfig{Socket: "/var/run/dolt.sock"})
		require.NoError(t, err)
		dsn := s.DSN(context.Background(), "beads", "root", "")
...
func TestExternalDoltServer_Dial(t *testing.T) {
	t.Run("tcp dial against a live listener succeeds", func(t *testing.T) {
		...
		s, err := NewExternalDoltServer(ExternalDoltConfig{Host: host, Port: port})
		require.NoError(t, err)
	...
	t.Run("tcp dial against a closed listener returns wrapped error", func(t *testing.T) {
		...
		s, err := NewExternalDoltServer(ExternalDoltConfig{Host: "127.0.0.1", Port: addr.Port})
		require.NoError(t, err)
		require.NoError(t, s.Start(context.Background()))
		t.Cleanup(func() { _ = s.Stop(context.Background()) })
	...
	t.Run("dial honors a canceled context", func(t *testing.T) {
		s, err := NewExternalDoltServer(ExternalDoltConfig{Host: "10.255.255.1", Port: 9999})
		require.NoError(t, err)
		require.NoError(t, s.Start(context.Background()))
		t.Cleanup(func() { _ = s.Stop(context.Background()) })
