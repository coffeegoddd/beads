...
// buildPageIDSubquery reproduces, as a subquery over ids alone, the row set
// searchAcrossIssuesAndWisps would return for the same (query, filter). It
// deliberately errs toward a superset: every branch that could touch wisps
// builds the union, so a caller joining against it can over-fetch a row but
// never miss one.
